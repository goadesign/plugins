...
var ClientMethodsWithPayloadCode = `// WithPayloadMethod calls the WithPayloadMethod method using the configured
// transport.
func (c *Client) WithPayloadMethod(ctx context.Context, p *withpayloadservice.WithPayloadMethodPayload) error {
	...
	case HTTPTransport:
		...
		endpoint := c.httpClient.WithPayloadMethod()
		_, err := endpoint(ctx, p)
		return err
	case GRPCTransport:
		if c.grpcClient == nil {
			return fmt.Errorf("gRPC transport not configured")
		}
		endpoint := c.grpcClient.WithPayloadMethod()
		_, err := endpoint(ctx, p)
		return err
	case JSONRPCTransport:
		...
		endpoint := c.jsonrpcClient.WithPayloadMethod()
		_, err := endpoint(ctx, p)
		return err

	default:
		return fmt.Errorf("no transport available for WithPayloadMethod")
	}
}
`
...
func (r *ScenarioRunner) executeMethod(ctx context.Context, client *Client, method string, payload map[string]any) (any, error) {
	switch method {
	case "WithPayloadMethod":
		// Convert payload map to typed payload
		p := &withpayloadservice.WithPayloadMethodPayload{}
		if err := r.mapToStruct(payload, p); err != nil {
			return nil, fmt.Errorf("invalid payload for WithPayloadMethod: %w", err)
		}
		return nil, client.WithPayloadMethod(ctx, p)
	default:
...
func RunWithPayloadServiceHarness(t *testing.T, svc withpayloadservice.Service) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	h := withPayloadServicetest.NewHarness(t, svc)
	defer h.Close()

	td := withPayloadServicetest.NewTestData()
	t.Run("WithPayloadMethod", func(t *testing.T) {
		err := h.Client.WithPayloadMethod(ctx, td.ValidWithPayloadMethodPayload())
		if err != nil {
			t.Errorf("WithPayloadMethod failed: %v", err)
		}
	})
}
`
