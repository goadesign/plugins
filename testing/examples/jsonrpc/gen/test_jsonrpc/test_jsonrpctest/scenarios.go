...
//
// Scenario runner for test-jsonrpc service
//
// Command:
...
func (r *ScenarioRunner) executeMethod(ctx context.Context, client *Client, method string, payload map[string]any) (any, error) {
	...
	case "jsonrpc_no_stream":
		...
		p := &testjsonrpc.JsonrpcNoStreamPayload{}
		if err := r.mapToStruct(payload, p); err != nil {
			return nil, fmt.Errorf("invalid payload for jsonrpc_no_stream: %w", err)
		}
		return client.JsonrpcNoStream(ctx, p)
	case "jsonrpc_no_stream_error":
		// Convert payload map to typed payload
		p := &testjsonrpc.JsonrpcNoStreamErrorPayload{}
		if err := r.mapToStruct(payload, p); err != nil {
			return nil, fmt.Errorf("invalid payload for jsonrpc_no_stream_error: %w", err)
		}
		return client.JsonrpcNoStreamError(ctx, p)
	default:
...
func (r *ScenarioRunner) callValidator(t *testing.T, method string, result any, expect Expectation) {
	...
	switch method {
	case "jsonrpc_no_stream":
		typedResult := result.(*testjsonrpc.JsonrpcNoStreamResult)
	...
	case "jsonrpc_no_stream_error":
		typedResult := result.(*testjsonrpc.JsonrpcNoStreamErrorResult)
		_ = typedResult // no validators defined in YAML
		t.Errorf("validator %q specified but not generated - add it to scenarios.yaml first", validatorName)
	default:
