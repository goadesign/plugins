...
import (
	...
	testhttpgrpcsvr "goa.design/plugins/v3/testing/examples/httpgrpc/gen/http/test_http_grpc/server"
	testhttpgrpc "goa.design/plugins/v3/testing/examples/httpgrpc/gen/test_http_grpc"
)

// handleHTTPServer starts configures and starts a HTTP server on the given
// URL. It shuts down the server if any error is received in the error channel.
func handleHTTPServer(ctx context.Context, u *url.URL, testHTTPGrpcEndpoints *testhttpgrpc.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	)
	{
		...
		testHTTPGrpcServer = testhttpgrpcsvr.New(testHTTPGrpcEndpoints, mux, dec, enc, eh, nil, upgrader, nil)
	}

	// Configure the mux.
	testhttpgrpcsvr.Mount(mux, testHTTPGrpcServer)
	...
	for _, m := range testHTTPGrpcServer.Mounts {
