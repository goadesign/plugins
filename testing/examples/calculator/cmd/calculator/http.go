...
import (
	...
	calculatorsvr "goa.design/plugins/v3/testing/examples/calculator/gen/http/calculator/server"
)

// handleHTTPServer starts configures and starts a HTTP server on the given
// URL. It shuts down the server if any error is received in the error channel.
func handleHTTPServer(ctx context.Context, u *url.URL, calculatorEndpoints *calculator.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	// responses.
	var (
		calculatorServer *calculatorsvr.Server
	)
	{
		eh := errorHandler(ctx)
		upgrader := &websocket.Upgrader{}
		calculatorServer = calculatorsvr.New(calculatorEndpoints, mux, dec, enc, eh, nil, upgrader, nil)
	}

	// Configure the mux.
	calculatorsvr.Mount(mux, calculatorServer)
	...
	for _, m := range calculatorServer.Mounts {
