...
func Generate(files []*codegen.File) {
	for _, f := range files {
		// Make sure that generated code that calls request decoder
		// calls the one generated by this plugin.
		for _, s := range f.SectionTemplates {
			if s.Name == "request-decoder" {
				continue
			}
			s.Source = strings.Replace(s.Soure, "{{ .RequestDecoder }}", "{{ .SecureRequestDecoder }}", -1)
		}
...
func generateToken(data *httpcodegen.EndpointData, scheme *design.SchemeExpr, tokenField string) *codegen.SectionTemplate {
	...
	return &codegen.SectionTemplate{
		Name:    "secure-request-decoder-token",
...
func {{ .SecureRequestDecoder }}(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (interface{}, error) {
	return func(r *http.Request) (interface{}, error) {
		payload, err := {{ .RequestDecoder }}(mux, decoder)
		if err != nil {
			return nil, err
		}
		u, p, ok := r.BasicAuth()
