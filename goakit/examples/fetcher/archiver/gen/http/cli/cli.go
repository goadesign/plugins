...
import (
	...
	archiverc "goa.design/plugins/goakit/examples/client/archiver/gen/http/archiver/client"
	healthc "goa.design/plugins/goakit/examples/client/archiver/gen/http/health/client"
)
...
func ParseEndpoint(
	...
	)
	{
		...
		case "archiver":
			...
			case "archive":
				endpoint = c.Archive()
				data, err = archiverc.BuildArchiveArchivePayload(*archiverArchiveBodyFlag)
			case "read":
				endpoint = c.Read()
				data, err = archiverc.BuildReadReadPayload(*archiverReadIDFlag)
			}
		case "health":
