...
func fetcherUsage() {
	fmt.Fprintln(os.Stderr, `Service is the fetcher service interface.`)
	fmt.Fprintf(os.Stderr, "Usage:\n    %s [globalflags] fetcher COMMAND [flags]\n\n", os.Args[0])
	fmt.Fprintln(os.Stderr, "COMMAND:")
	fmt.Fprintln(os.Stderr, `    fetch: Fetch makes a GET request to the given URL and stores the results in the archiver service which must be running or the request fails`)
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Additional help:")
	fmt.Fprintf(os.Stderr, "    %s fetcher COMMAND --help\n", os.Args[0])
}
func fetcherFetchUsage() {
	// Header with flags
	fmt.Fprintf(os.Stderr, "%s [flags] fetcher fetch", os.Args[0])
	fmt.Fprint(os.Stderr, " -url STRING")
	fmt.Fprintln(os.Stderr)

	// Description
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, `Fetch makes a GET request to the given URL and stores the results in the archiver service which must be running or the request fails`)

	// Flags list
	fmt.Fprintln(os.Stderr, `    -url STRING: URL to be fetched`)
